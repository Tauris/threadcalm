...
- **Opens whole threads.** Clicks reply counters, "show previous comments" and "see more"
  until the conversation is fully open — then stops. It never clicks menus.
...
- **Works in any interface language** for thread expansion — reply counters are recognised by
...
`j` / `k` next and previous post · `c` copy the thread · `y` copy its link · `o` expand ·
`e` pause expansion · `a` hide the action bars · `r` reading mode · `s` settings · `?` all shortcuts
...
- **Stored locally only** — your settings, and a capped list of post identifiers for the
  "new post" marker.
...
- [Full guide](https://github.com/Tauris/threadcalm/blob/main/docs/USAGE.md) — every setting, with its default
- [Source and releases](https://github.com/Tauris/threadcalm)
- [Report a problem](https://github.com/Tauris/threadcalm/issues) — please include the exact
  text of the control involved, and never paste colleagues' names or post content

BSD 3-Clause © 2026 Jörg Türmer
