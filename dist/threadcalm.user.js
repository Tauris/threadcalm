...
(() => {
      ...
      overlay = el(
        ...
        el(
          ...
          el(
            ...
            el("span", {
              text: "Shortcuts are ignored while you are typing. Press ? or use the panel’s ? button to see this again. "
            }),
  ...
  function createTranslateTamer({ matchers }) {
    ...
    function classify(element) {
      const texts = accessibleTexts(element).filter((text) => text.length <= MAX_LABEL_LENGTH3);
      if (texts.length === 0) return null;
  ...
  function createPanel({ expander, highlighter, version }) {
    let panel = null;
    let statusText = null;
